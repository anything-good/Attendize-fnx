...
Route::get('/migrate', function () {
    // Security checks should be here. Never expose such functionality without proper authentication and authorization.
    try {
        Artisan::call('migrate', [
            '--force' => true, // Use --force to run migrations in production without confirmation
        ]);
        $output = Artisan::output();

        return new Response($output, 200);
    } catch (\Exception $e) {
        return new Response('Failed to run migrations: '.$e->getMessage(), 500);
    }
});
Route::get('/seed', function () {
    // Ensure this route is protected by authentication and authorization checks

    try {
        Artisan::call('db:seed', [
            '--force' => true, // This option is required to run the seeders in production
        ]);
        $output = Artisan::output();

        return new Response('Database seeding completed successfully. Output: ' . $output, 200);
    } catch (\Exception $e) {
        return new Response('Failed to seed database: ' . $e->getMessage(), 500);
    }
});

Route::get('/generate-key', function (Request $request) {
    // Optionally, you could add some authentication mechanism here
    // to ensure that not just anyone can trigger this.
 Artisan::call('key:generate', [
        '--show' => true, // This option will return the generated key instead of setting it in the .env file
    ]);
    Artisan::call('key:generate', [
        '--show' => true, // This option will return the generated key instead of setting it in the .env file
    ]);
